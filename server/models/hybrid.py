...
def load_hybrid_model(model_path, label_encoders, device):
    model = HybridFusionMultiTask(
...
# Load tokenizers for Hybrid model
def load_hybrid_tokenizers(tokenizer_dir):
    gpt2_tokenizer = GPT2TokenizerFast.from_pretrained(f"{tokenizer_dir}/gpt2_tokenizer")
    distilbert_tokenizer = DistilBertTokenizerFast.from_pretrained(f"{tokenizer_dir}/distilbert_tokenizer")
    return gpt2_tokenizer, distilbert_tokenizer
